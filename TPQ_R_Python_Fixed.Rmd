...
calc_proj_mat <- function(df_group, Factor, max_components=NA){
  ...
  # make sure both data frameshave the same subjects, and choose the IC columns
...
GAD_ICs = rownames(GAD_proj_mat)[apply(abs(GAD_proj_mat),1,max)>0.7]
MDD_ICs = rownames(MDD_proj_mat)[apply(abs(MDD_proj_mat),1,max)>0.9]
HC_ICs = rownames(HC_proj_mat)[apply(abs(HC_proj_mat),1,max)>0.9]
PTSD_ICs = rownames(PTSD_proj_mat)[apply(abs(PTSD_proj_mat),1,max)>0.6]
TNP_ICs = rownames(TNP_proj_mat)[apply(abs(TNP_proj_mat),1,max)>0.7]

CorProj = as.matrix(cor(df_all,df_group))
CorProj[abs(CorProj)<0.6] = 0
print(CorProj)

CorSource = list()
for (i in 1:ncol(sources_all)){
  Cors = sapply(1:nrow(df_all), function(x) cor.test(as.numeric(abs(sources_all[i,][as.numeric(sources_all[i,])>as.numeric(quantile(sources_all[i,],0.5))])), as.numeric(abs(sources_mdd[x,][as.numeric(sources_all[i,])>as.numeric(quantile(sources_all[i,],0.5))])), method = "spearman")$estimate)
  CorSource = append(CorSource,as.vector(Cors))
}
