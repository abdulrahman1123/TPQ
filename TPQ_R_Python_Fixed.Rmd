...
multiplot(gs_fast, Listing = TRUE, cols = 3)
...
```{r Check questions that contribute to each IC}
library(reshape2)
sources_all = py$sources_all

# convert projections to numeric
ic_count = nrow(sources_all)
sources_all$IC = paste0("IC",1:ic_count)
sources_melted = melt(sources_all, id.vars = "IC", measure.vars = paste0("Q",1:100), value.name = "Source", variable.name = "Question")
ggplot(sources_melted, aes(x= Question, y= Source, group = IC))+
  geom_line()+
  facet_wrap(~IC)



sources_means = SMeans(sources_melted, "Source", "Question", GroupBy = "Question")
sources_means$Projection = abs(sources_means$Projection)
sources_means$IC = factor(sources_means$IC, levels = paste0("IC",1:nlevels(sources_melted$IC)))
for (IC in paste0("IC",1:nlevels(sources_means$IC))){
  sources_means$Max[sources_means$IC==IC] = max(sources_means$Projection[sources_means$IC==IC])
}
sources_means$Max_group = ifelse(sources_means$Max == sources_means$Projection,"*","")

# calculate the difference for each one
for (IC in paste0("IC",1:nlevels(sources_means$IC))){
  for (lvl in levels(factor(sources_means[[Factor]]))){
    sources_means$Difference[sources_means$IC==IC&sources_means[[Factor]]==lvl] = sum(abs(sources_means$Projection[sources_means$IC==IC&sources_means[[Factor]]==lvl] - sources_means$Projection[sources_means$IC==IC&sources_means[[Factor]]!=lvl]))
  }
}

# find the largest difference
for (IC in paste0("IC",1:nlevels(sources_means$IC))){
  sources_means$MDif[sources_means$IC==IC] = max(sources_means$Difference[sources_means$IC==IC])
}

sources_means$RDif = round(sources_means$Difference/sources_means$MDif,2)
sources_means$RDif_text = paste(sources_means[[Factor]]," = ",sources_means$RDif)

# delete those with less than 0.5 difference
sources_means$RDif_text[sources_means$RDif<0.75] = ""
sources_means$Inclusion = ifelse(sources_means$RDif>0.75,"*","")

g1 = ggplot(sources_means, aes(x=Groups, y=Projection, group = Groups, fill = Groups))+
  geom_bar(stat="identity", color = "black")+
  geom_errorbar(aes(ymin = Projection-SEM,ymax = Projection+SEM), width = 0.15)+
  facet_grid(~IC)+
  scale_fill_manual(values = c("#4EA3DF","#6cBE58","#C33E3B","#808CA3","#B9B0AB"))+
  TypicalTheme+
  geom_text(aes(y = ifelse(sources_means$Projection>0,sources_means$Projection+0.007,sources_means$Projection-0.007),color = Groups, label=Max_group),size =15)+
  scale_color_manual(values = c("#4EA3DF","#6cBE58","#C33E3B","#808CA3","#B9B0AB"))+
  ggtitle("Absolute Values for IC Sources for 5 Groups of Subjects", subtitle = "Stars indicate groups with largest differences")
