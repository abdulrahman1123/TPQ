...
```{r understand how to get reconstructed data from sources}
sources = as.matrix(sources_all)
projections = projections_all[,9:23]
projections = apply(projections,2,as.numeric)
projections =as.matrix(projections)
orig_data = data_tpq_all[9:108]
IC1_pro = matrix(orig_data[1,],ncol = 1)%*%matrix(projections[,1],nrow = 1)

IC1_reco = data_reco_all[[1]][,9:108]
IC1_orig = t(as.matrix(orig_data))%*%projections

#multiply sources with the inverse of original data to get the mixing matrix
sources%*%inv(t(orig_data))

# Conclusion: I do not have the full mixing matrix. I will only use the reco data
sources
```
...
calc_per <- function(Factor){
  ...
  fig = ggplot(percentage)+
    ...
    TypicalTheme+
    ggtitle("Percentage of Subject Answers to TPQ Questions", subtitle = paste0("For ",Factor," Group"))+
    theme(axis.text.x = element_text(angle = 90))+
    facet_wrap(~Arrangement,scales = "free",ncol=1)
 
  
  return(list(percentage, fig))
}
...
multiplot(all_true[[2]],all_true[[3]], cols=1)
multiplot(mdd_true[[2]],mdd_true[[3]], cols=1)
multiplot(gad_true[[2]],gad_true[[3]], cols=1)
multiplot(ptsd_true[[2]],ptsd_true[[3]], cols=1)
multiplot(trauma_true[[2]],trauma_true[[3]], cols=1)

percentages = data.frame(Question = rep(1:100,2), Answer = rep(c("True","False"), each = 100), All = all_true[[1]]$Percentage,
                         MDD = mdd_true[[1]]$Percentage, GAD = gad_true[[1]]$Percentage, PTSD = ptsd_true[[1]]$Percentage, TNP = trauma_true[[1]]$Percentage)
percentages_melted = melt(percentages, id.vars = c("Question","Answer"), value.name = "Percentage", variable.name = "Group")
percentages_melted$Arranged_questions = Arranged_questions
percentages_melted[order(percentages_melted$Arranged_questions),]
percentages_melted$Question = paste0("Q",percentages_melted$Question)



percentages_melted = percentages_melted[order(percentages_melted$Arranged_questions),]
NewQuestion = percentages_melted$Question[order(percentages_melted$Arranged_questions,percentages_melted$Question)]
NewPerc = percentages_melted$Percentage[order(percentages_melted$Arranged_questions,percentages_melted$Percentage)]
NewAnswer = percentages_melted$Answer[order(percentages_melted$Arranged_questions,percentages_melted$Answer)]
NewGrouping = percentages_melted$Group[order(percentages_melted$Arranged_questions,percentages_melted$Group)]

ggplot(percentages_melted[percentages_melted$Group=="All",])+
  geom_bar(aes(x=Question, y=Percentage, group = Answer, fill = Answer),stat= "identity")+
  #geom_bar(mapping=aes(x=Question, y=Percentage, group = Answer, fill = Answer),stat= "identity")+
  scale_fill_manual(values = c("#808CA3","#B9B0AB"))+
  TypicalTheme
