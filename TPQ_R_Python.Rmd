...
or_scores_path = "/home/abdulrahman/abdulrahman.sawalma@gmail.com/PhD/Data/Palestine/TPQ_DataAndAnalysis/TPQ_Analysis_All_25.11.2020_modified.xlsx"
if os.path.exists("/home/asawalma/"):
  path = path.replace("/home/abdulrahman/abdulrahman.sawalma@gmail.com","/home/asawalma/Insync/abdulrahman.sawalma@gmail.com/Google Drive")
  or_scores_path = or_scores_path.replace("/home/abdulrahman/abdulrahman.sawalma@gmail.com","/home/asawalma/Insync/abdulrahman.sawalma@gmail.com/Google Drive")
...
df = pd.read_excel(or_scores_path)
sub_inds =[]
for i in range(len(npz_ids)):
  if npz_ids[i] in df["Final ID"].tolist():
    sub_inds.append(np.where(df["Final ID"] == npz_ids[i])[0][0])

groups = [df["Diagnosis"][item] for item in sub_inds]
...
```{r load all data for further analysis (RE-DONE) , warning=FALSE, cache=TRUE}
library (ppcor)
library(readxl)
library(reshape2)
library(ggplot2)
...
#read the data
score_parent = "/home/abdulrahman/abdulrahman.sawalma@gmail.com/PhD/Data/Palestine/TPQ_DataAndAnalysis/"
...
create_weights <- function(data_frame) {
  ...
  original_scores = scores[match("QO1", colnames(scores)):match("QO100", colnames(scores))]
  loadings = data_frame[match("Q1_load", colnames(data_frame)):match("Q100_load", colnames(data_frame))]
  colnames(loadings) =colnames(original_scores)
  
  weighted_df = original_scores * loadings
  
  weighted_df[c("ID", "Diagnosis")] = data_frame[c("ID", "Diagnosis")]
  original_scores[c("ID", "Diagnosis")] = data_frame[c("ID", "Diagnosis")]
  loadings[c("ID", "Diagnosis")] = data_frame[c("ID", "Diagnosis")]
  
  final_df = rbind(original_scores,weighted_df,loadings)
  final_df$Score_type = rep(c("original","weight","loading"),each = nrow(original_scores))
  colnames(final_df) = c(sapply(1:100,function(x) paste0("Q",x)),"ID","Diagnosis","Score_type")
...
create_array <- function(data_frame) {
  # concatenate all data frames into one big dataframe
  fast_df_all = Reduce(rbind,data_frame)
  
  # add the column IC. This is the column based on which categorization will be done.
  fast_df_all$IC = unlist(sapply(1:length(data_frame), function(ic_num) rep(paste0("IC", ic_num), nrow(data_frame[[1]])), simplify = FALSE))
  
  # create the final array. It should have the dimensions of nrow, ncols, num_of_ICs
  ar = simplify2array(by(fast_df_all,fast_df_all$IC,as.matrix))
  
  # return the data frame
  return (ar)
}

fast_array = create_array(weighted_fast_list)
noboot_fast_array = create_array(weighted_noboot_fast_list)
info_array = create_array(weighted_info_list)
...
colnames(weighted_fast) = gsub("QO", "Q",colnames(weighted_fast))
...
sapply(1: length(py$data_fast), function(x)
  ggsave(paste0(fig_dir,"/Q_Threshold_FastICA_Boot_",x,".png"),
    plot = plot_questions(data_fast[[x]], plotting = "loadings"), width = 570,
...
cor_2df = function(df1,df2){
  p_matrix = matrix(nrow = 100, ncol = 100)
  rownames(p_matrix) = paste0("QO",1:100)
  colnames(p_matrix) = paste0("QO",1:100)
  
  r_matrix = matrix(nrow = 100, ncol = 100)
  rownames(r_matrix) = paste0("QO",1:100)
  colnames(r_matrix) = paste0("QO",1:100)
  
  for (item1 in weighted_col_list){
    for (item2 in weighted_col_list){
      est = cor.test(df1[[item1]],df2[[item2]])$estimate
      pval = cor.test(df1[[item1]],df2[[item2]])$p.value
      p_matrix[item1,item2] = pval
      r_matrix[item1,item2] = est
    }
  }
  
  # change the questions back to become Q1, Q2, Q3 ... etc. instead of QO1, QO2, QO3... etc.
  rownames(p_matrix) = sapply(rownames(p_matrix), function(x) gsub("QO","Q",x))
  colnames(p_matrix) = sapply(colnames(p_matrix), function(x) gsub("QO","Q",x))
  
  rownames(r_matrix) = sapply(rownames(r_matrix), function(x) gsub("QO","Q",x))
  colnames(r_matrix) = sapply(colnames(r_matrix), function(x) gsub("QO","Q",x))
  
  final_list = list (p = p_matrix, r = r_matrix)
  
  
  # Insignificant correlation are crossed
  c1 = corrplot(final_list$r, type="upper",p.mat = final_list$p, sig.level = 0.01, insig = "blank")
  return (c1)
}
...
cor_2df(df_fast_list[[2]],df_fast_list[[3]])
